...
public final class OriginButtonRenderer {
	...
	private static void render0(Gfx guiGraphics, AbstractButton button) {
		ensureLoaded();
		// 1.19.2: position is the public x/y fields (getX/getY are 1.19.3+), and
		// there is no isHovered() method — isHoveredOrFocused() is this era's
		// highlight test (it's what vanilla's own widget draw brightens on).
		int x = button.x, y = button.y, w = button.getWidth(), h = button.getHeight();
		boolean enabled = button.active;
		double hv = hoverEase(button, enabled && (button.isHovered() || button.isFocused()));
	...
	private static void renderSlider0(Gfx guiGraphics, AbstractSliderButton slider, double value) {
		ensureLoaded();
		int x = slider.x, y = slider.y, w = slider.getWidth(), h = slider.getHeight();
		boolean enabled = slider.active;
		double v = Math.max(0.0, Math.min(1.0, value));
		double hv = hoverEase(slider, enabled && (slider.isHovered() || slider.isFocused()));
	...
	private static void renderCheckbox0(Gfx guiGraphics, Checkbox checkbox) {
		ensureLoaded();
		int x = checkbox.x, y = checkbox.y, h = checkbox.getHeight();
		boolean enabled = checkbox.active;
		double hv = hoverEase(checkbox, enabled && (checkbox.isHovered() || checkbox.isFocused()));
