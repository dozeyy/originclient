...
public final class OriginSdfFont {
	...
	private static final class Face {
		...
		Face(String texName) {
			this.atlas = new ResourceLocation("originclient", texName);
		}
	}
	...
	private static void loadFace(Face f, String base) {
		try {
			...
			for (java.util.Map.Entry<String, com.google.gson.JsonElement> e : g.entrySet()) {
				String k = e.getKey();
				if (k.isEmpty()) {
					continue;
				}
	...
	public static void draw(Gfx g, String text, float x, float y, int argb, boolean shadow, boolean bold) {
	...
	private static void emit(Gfx g, Face f, String text, float x, float y, int argb) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if (gl.w > 0 && gl.h > 0) {
				...
				bb.vertex(m, gx0, gy0, 0).color(r, gg, b, a).uv(u0, v0).endVertex();
				bb.vertex(m, gx0, gy1, 0).color(r, gg, b, a).uv(u0, v1).endVertex();
				bb.vertex(m, gx1, gy1, 0).color(r, gg, b, a).uv(u1, v1).endVertex();
				bb.vertex(m, gx1, gy0, 0).color(r, gg, b, a).uv(u1, v0).endVertex();
				any = true;
			}
			pen += gl.adv * s;
		}
