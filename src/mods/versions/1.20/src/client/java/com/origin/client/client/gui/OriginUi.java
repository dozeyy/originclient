...
public final class OriginUi {
	 ...
	 * editor, the colour picker — draws through here, so one implementation
	 * rounds all of them consistently by construction.
	 *
	 ...
	 * with a small corner instead of the panel radius. The 45° chamfer is gone —
	 * buttons, cards and chips are rounded like everything else — so this just
	 * forwards with the cut used as the radius.
	 */
	...
	private static ResourceLocation boxMask(int w, int h, int r) {
		...
		try {
			...
			// Path must stay [a-z0-9_./-] — anything else is an illegal ResourceLocation.
			id = new ResourceLocation("originclient", "textures/ui/origin_box_" + w + "x" + h + "_" + r + "_s" + s);
			DynamicTexture tex = new DynamicTexture(img);
			// NEAREST: the blit is 1:1 texel-to-screen-pixel, so filtering would only
			// soften an already exact result.
	...
	private static ResourceLocation cornerMask(int r, boolean ring) {
		...
		try {
			...
			id = new ResourceLocation("originclient",
					"textures/ui/origin_corner_" + (ring ? "ring" : "fill") + "_" + r + "_s" + s);
			DynamicTexture tex = new DynamicTexture(img);
			tex.setFilter(false, false);
			Minecraft.getInstance().getTextureManager().register(id, tex);
		} catch (Throwable t2) {
