...
public final class HudElements {
	...
	private static List<String> coordsLines(Minecraft mc) {
		...
		if (Mods.bool("coords", "renderers")) {
			int c;
			try {
				c = mc.levelRenderer.countRenderedChunks();
			} catch (Throwable t) {
				c = -1;
			}
	...
	private record PotionRow(MobEffect effect, int duration, boolean infinite) {
	}

	private static int rowTextWidth(Minecraft mc, PotionRow row) {
		String name = row.effect.getDisplayName().getString();
	...
	private static void drawEffectRow(GuiGraphics g, Minecraft mc, int y, MobEffect effect, int duration, boolean infinite) {
		...
		try {
			...
			g.fill(2, y + 4, 12, y + 14, 0xFF000000 | effect.getColor());
		}
