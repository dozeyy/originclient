...
public final class OriginSdfFont {
	...
	private static void emit(GuiGraphics g, Face f, String text, float x, float y, int argb) {
		...
		// NOT SUBMITTED ON 1.21.5 — see the note on OriginShaders.enabled().
		//
		// Everything above (atlas parse, glyph layout, quad build) is correct and
		// version-independent; only the SUBMISSION is era-specific. 1.21.5 removed
		// the immediate-mode path this needs: there is no RenderSystem.setShader,
		// no enableBlend/defaultBlendFunc, and the GUI is moving to RenderPipeline
		// (which 1.21.5 has, but WITHOUT the RenderPipelines registry that arrives
		// in 1.21.6, so 1.21.6's approach doesn't port back either). Finishing it
		// means building a RenderPipeline by hand and finding this version's own
		// way to submit GUI quads with it — a real port, not a retarget.
		//
		// Until then active() is false, so callers never reach this and text draws
		// through the vanilla font exactly as the shipped 1.21.5 build does today.
		mesh.close();
	}
}
