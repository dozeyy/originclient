...
public final class OriginScreenRenderer {
	...
	public static boolean renderTitleBackground(GuiGraphics guiGraphics) {
		...
		try {
			...
			// smoothly on menus that don't advance game time. (1.21.5 renamed
			// Minecraft.getTimer() to getDeltaTracker().)
			float partial = mc.getDeltaTracker().getGameTimeDeltaPartialTick(false);

			PANORAMA.render(guiGraphics, w, h, 1.0f, partial);

			// Blur exactly the way this era's vanilla does it: 1.21.5's
			// Screen.renderBlurredBackground() is nothing but this one call (checked
			// against the jar's bytecode) — no partial tick, and no render-target
			// rebind, because this era's blur no longer leaves a foreign target bound.
