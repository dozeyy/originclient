...
public final class BlockOverlayRenderer {
	...
	public static boolean onBlockOutline(WorldRenderContext wctx, BlockOutlineRenderState octx) {
		...
		// Render state's short ctor only fills shape(); interactionShape() is NULL
		// in real gameplay (the silently-swallowed NPE that killed the outline).
		...
		if (outline) {
			...
			VertexConsumer lines = wctx.consumers().getBuffer(RenderType.lines());
