 ...
 *
 * (Pre-1.19.3 era: the registry is Registry.PARTICLE_TYPE, not BuiltInRegistries;
 * 1.16.5 also needs the partial-tick overload of getEyePosition.)
 *
...
public final class ParticleFilter {
	...
	public static String typePath(ParticleOptions options) {
		net.minecraft.resources.ResourceLocation key = Registry.PARTICLE_TYPE.getKey(options.getType());
	...
	public static boolean hidden(ParticleOptions options, double x, double y, double z) {
		...
		if (Mods.bool("particles", "hideFirstPerson")) {
			...
			if (mc.player != null && mc.options.getCameraType().isFirstPerson()
					&& mc.player.getEyePosition(1.0F).distanceToSqr(x, y, z) < 4.0) {
				return true;
			}
		}
		...
		if (mode.equals("Reduced")) {
			net.minecraft.core.particles.ParticleType<?> t = options.getType();
