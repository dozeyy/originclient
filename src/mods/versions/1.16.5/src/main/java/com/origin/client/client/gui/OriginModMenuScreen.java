...
public class OriginModMenuScreen extends Screen {
	...
	private static final class SRow {
		private final ModOption o;
		private final int y, h;
		private final boolean indent;

		private SRow(ModOption o, int y, int h, boolean indent) {
			this.o = o;
			this.y = y;
			this.h = h;
			this.indent = indent;
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		if (page != null) {
			...
			switch (nav) {
				case MODS:
					renderMods(g, mouseX, mouseY, now, t);
					break;
				case SETTINGS:
					renderSettingsPage(g, mouseX, mouseY, now, t);
					break;
			}
		}
		pose.popPose();
	...
	private void renderRow(Gfx g, String modId, ModOption o, int x0, int x1, int y, int mx, int my, float alpha) {
		...
		switch (o.kind) {
			case TOGGLE:
				OriginUi.switchAt(g, modId + ":" + o.key, x1 - 40, y + 5, 30, vBool(modId, o.key), true);
				break;
			case SLIDER: {
