...
public class OriginClientMod implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		net.fabricmc.fabric.api.client.screen.v1.ScreenEvents.AFTER_INIT.register((client, screen, sw, sh) -> {
			if (screen.getClass().getName().contains("ShaderPackScreen")) {
				java.util.List<net.minecraft.client.gui.components.AbstractWidget> buttons = net.fabricmc.fabric.api.client.screen.v1.Screens.getButtons(screen);
				...
				boolean present = buttons.stream().anyMatch(w ->
						w instanceof net.minecraft.client.gui.components.Button
								&& label.equals(((net.minecraft.client.gui.components.Button) w).getMessage()));
				if (!present) {
			...
			net.fabricmc.fabric.api.client.screen.v1.ScreenEvents.beforeRender(screen).register((s, poseStack, mx, my, tick) -> {
				try {
					...
					if (com.origin.client.client.render.OriginScreenRenderer.renderTitleBackground(g)) {
						...
						for (net.minecraft.client.gui.components.events.GuiEventListener child : s.children()) {
							if (child instanceof net.minecraft.client.gui.components.AbstractWidget
									&& ((net.minecraft.client.gui.components.AbstractWidget) child).visible && ((net.minecraft.client.gui.components.AbstractWidget) child).isMouseOver(mx, my)) {
								hovering = true;
								break;
							}
						}
	...
	private void onEndTick(Minecraft client) {
		...
		if (Mods.on("togglesprint") && Mods.bool("togglesprint", "flyBoost")
				&& player.abilities.flying && (player.isCreative() || player.isSpectator())) {
			float mult = (float) Math.max(1.0, Mods.num("togglesprint", "flyBoostAmount"));
			player.abilities.setFlyingSpeed(0.05f * mult);
