 ...
 * etc.). Kind is detected structurally, verified against jei-1.21.1-fabric-
 * 19.27.0.340 with javap (not assumed):
 ...
 *
 * 1.21.1-ONLY. Only 1.21.1 bundles JEI, so this file references mezz.jei.* and
...
public final class JeiSettings {
	...
	private static ModOption build(String category, IJeiConfigValue<?> v) {
		try {
			...
			String label = v.getLocalizedName().getString();
			...
			HANDLES.put(key, v);
			String tip = v.getLocalizedDescription().getString();
