...
public final class OriginButtonRenderer {
	...
	// 1.19.3 AbstractWidget has no isHovered() accessor (that arrived in
	// 1.19.4) -- only the protected isHovered field set during render. The
	// public isMouseOver(double,double) does the same visible/active/bounds
	// test, so hover is recomputed from the live cursor in GUI space (the
	// exact xpos*guiWidth/screenWidth mapping vanilla uses for the render-pass
	// mouseX/mouseY). Used by every hover check in the module.
	public static boolean hovered(AbstractWidget w) {
		Minecraft mc = Minecraft.getInstance();
		double mx = mc.mouseHandler.xpos() * mc.getWindow().getGuiScaledWidth() / mc.getWindow().getScreenWidth();
		double my = mc.mouseHandler.ypos() * mc.getWindow().getGuiScaledHeight() / mc.getWindow().getScreenHeight();
		return w.isMouseOver(mx, my);
	}
	...
	private static void render0(Gfx guiGraphics, AbstractButton button) {
		ensureLoaded();
		int x = button.getX(), y = button.getY(), w = button.getWidth(), h = button.getHeight();
		boolean enabled = button.active;
		double hv = hoverEase(button, enabled && hovered(button));
	...
	private static void renderTab0(Gfx guiGraphics, Object key, int x, int y, int w, int h,
								   Component label, boolean selected, boolean hovered) {
		ensureLoaded();
	...
	private static void renderSlider0(Gfx guiGraphics, AbstractSliderButton slider, double value) {
		ensureLoaded();
		...
		double hv = hoverEase(slider, enabled && hovered(slider));
	...
	private static void renderCheckbox0(Gfx guiGraphics, Checkbox checkbox) {
		ensureLoaded();
		int x = checkbox.getX(), y = checkbox.getY(), h = checkbox.getHeight();
		boolean enabled = checkbox.active;
		double hv = hoverEase(checkbox, enabled && hovered(checkbox));
	...
	private static void drawLabel(Gfx guiGraphics, double cx, double cy, int h, Component message, int labelColor) {
	...
	private static void drawFallback(Gfx guiGraphics, int x, int y, int w, int h, int fill, int border, int labelColor, Component message) {
		guiGraphics.fill(x, y, x + w, y + h, fill);
	...
	private static void nineSlice(Gfx g, ResourceLocation tex, int x, int y, int w, int h, int cd) {
		int c = CORNER;
		int t = TEX;
		int mid = t - 2 * c;
		int mw = w - 2 * cd;
