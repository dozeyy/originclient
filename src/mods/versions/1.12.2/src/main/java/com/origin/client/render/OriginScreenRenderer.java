 ...
 * modern OriginScreenRenderer — this is the file that makes 1.12.2 look
 * exactly like 1.21.
 *
...
public final class OriginScreenRenderer {
    ...
    public static void drawCaption(double cx, double y, long elapsedMs) {
        ...
        for (int i = 0; i < word.length(); i++)
            wordW += mc.fontRenderer.getCharWidth(word.charAt(i)) + (i > 0 ? track : 0);
        int dotSlot = mc.fontRenderer.getCharWidth('.') + track;
        int total = wordW + dotSlot * 3;
        double x = cx - total / 2.0;
        for (int i = 0; i < word.length(); i++) {
            String ch = String.valueOf(word.charAt(i));
            mc.fontRenderer.drawString(ch, (float) x, (float) y, OriginTheme.MUTED, false);
            x += mc.fontRenderer.getCharWidth(word.charAt(i)) + track;
        }
        for (int i = 0; i < dots; i++) {
            mc.fontRenderer.drawString(".", (float) x, (float) y, OriginTheme.MUTED, false);
            x += dotSlot;
        }
    }
