...
public final class OriginTabList {
	...
	public static void render(GuiGraphics g, int screenWidth, List<PlayerInfo> allInfos,
		...
		if (combatOn) {
			list.sort((a, b) -> Long.compare(
					CombatTracker.lastHit(b.getProfile().getId(), combatMs),
					CombatTracker.lastHit(a.getProfile().getId(), combatMs)));
		}
		...
		for (int i = 0; i < n; i++) {
			...
			boolean fighting = combatOn
					&& CombatTracker.lastHit(info.getProfile().getId(), combatMs) > 0L;
			...
			if (showHealth) {
				drawHealth(g, font, mc, info.getProfile().getId(), x + cellW - pingW - scoreW - 2, ry);
			}
	...
	private static void drawHeartCell(GuiGraphics g, int x, int y, int fill) {
		g.blitSprite(net.minecraft.client.renderer.RenderType::guiTextured, HEART_CONTAINER, x, y, 9, 9);
		if (fill >= 2) {
			g.blitSprite(net.minecraft.client.renderer.RenderType::guiTextured, HEART_FULL, x, y, 9, 9);
		} else if (fill == 1) {
			g.blitSprite(net.minecraft.client.renderer.RenderType::guiTextured, HEART_HALF, x, y, 9, 9);
		}
	}
