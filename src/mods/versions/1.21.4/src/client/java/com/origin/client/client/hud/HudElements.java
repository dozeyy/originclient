...
public final class HudElements {
	...
	private static List<ItemStack> armorItems(Minecraft mc) {
		...
		if (mc.player != null) {
			// Armor list moved off Inventory; getItemBySlot works across the family.
	...
	private static void drawEffectRow(GuiGraphics g, Minecraft mc, int y, Holder<MobEffect> effect, int duration, boolean infinite) {
		...
		try {
			...
			// 1.21.2 blit rework: the TextureAtlasSprite overload dropped its
			// z/blitOffset arg and moved to blitSprite(RenderType, sprite, x, y, w, h).
			// Least-certain descriptor of the port — verify with javap; already
			// fail-soft (the catch below falls back to a flat color swatch).
			g.blitSprite(RenderType::guiTextured, sprite, 0, y, 18, 18);
		} catch (Throwable t) {
