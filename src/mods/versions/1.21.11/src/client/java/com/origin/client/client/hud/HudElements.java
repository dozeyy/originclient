...
public final class HudElements {
	...
	// backdrop) so the scoreboard preview element doesn't double-draw alongside
	// the menu's own content.
	...
	private static List<ItemStack> armorItems(Minecraft mc) {
		...
		if (mc.player != null) {
			// 1.21.11: armor moved from an Inventory list to EntityEquipment slots.
	...
	static {
		...
		// A real draggable element: default at the right edge, centred. In game it
		// draws NOTHING — the vanilla sidebar renders, repositioned and rescaled to
		// this same HudPos by GuiScoreboardMixin — so dragging the sample here moves
		// the real scoreboard.
	...
	private static void drawSampleScoreboard(GuiGraphics g) {
		...
		int bgc = OriginColorPicker.liveColor("scoreboard", "bgColor");
		int[] sz = sampleScoreboardSize();
		int boxW = sz[0], contentH = sz[1];
		int lineH = font.lineHeight + 1;
		OriginUi.panel(g, 0, 0, boxW, contentH, 0, bgc, 0);
	...
	private static void drawEffectRow(GuiGraphics g, Minecraft mc, int y, Holder<MobEffect> effect, int duration, boolean infinite) {
		...
		try {
			// 1.21.11: effect icons come from Gui.getMobEffectSprite (an Identifier,
			// not an atlas sprite) and blitSprite takes the RenderPipeline. Already
			// fail-soft (the catch below falls back to a flat color swatch).
			var sprite = mc.gui.getMobEffectSprite(effect);
			g.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, 0, y, 18, 18);
		} catch (Throwable t) {
