...
// camera → NDC → gui pixels). Because this is a GUI pass drawn after the world:
...
//     scale bars.
// WaypointRenderer.capture() refreshes the camera state every world frame; a
// waypoint behind the camera is simply not drawn — no edge markers.
//
// PER-VERSION DELTA (1.21.11): 1.21.1 captured the live ModelView/Projection
// matrices from RenderSystem and multiplied the world position through them.
// RenderSystem.getProjectionMatrix() no longer exists here (the projection lives
// in a GpuBufferSlice), so the projection is rebuilt from first principles
// instead: the camera's own orientation quaternion (published every frame in
// CameraRenderState) rotates the world delta into camera space, and the
// perspective divide uses the EFFECTIVE vertical FOV that GameRendererMixin
// publishes out of vanilla's private getFov. Same result, no engine matrices.
// The camera frame is vanilla's own (Camera.FORWARDS is (0,0,-1)), so forward is
// -Z, +X right, +Y up — a standard OpenGL frustum.
//
// Other era deltas: g.pose() is a Matrix3x2fStack (pushMatrix/popMatrix, 2-arg
// translate/scale), ResourceLocation -> Identifier, and blitSprite takes a
// RenderPipeline as its first argument.
public final class WaypointHud {
	...
	private static double camYawDeg;
	private static volatile boolean valid = false;

	/** Called from the world pass each frame with the live camera state. */
	static void capture(WorldRenderContext ctx) {
		...
		camY = p.y;
		camZ = p.z;
		ORIENT.set(cam.orientation);
		// Camera yaw in Minecraft's own convention (0 = +Z, increasing toward -X),
		// derived from the forward vector so it needs no Camera accessor — 1.21.11
		// dropped Camera.getYRot().
		Vector3f fwd = ORIENT.transform(new Vector3f(0f, 0f, -1f));
		camYawDeg = Math.toDegrees(Math.atan2(-fwd.x, fwd.z));
		valid = true;
	}
	...
	public static void renderBars(GuiGraphics g) {
		...
		if (!Mods.bool("waypoints", "separateBar")) {
			...
			pose.scale((float) pos.scale, (float) pos.scale);
	...
	// The XP bar is exactly 182×5 and sits at (sw-182)/2, sh-29 (vanilla
	// ContextualBarRenderer: left = (sw-182)/2, top = sh - 24 - 5).
	...
	public static void drawBar(GuiGraphics g, Minecraft mc, int bx, int by, int barW, boolean copyXpBar) {
		...
		if (copyXpBar) {
			g.blitSprite(RenderPipelines.GUI_TEXTURED, XP_BG, bx, by, barW, BAR_H);
		}
		...
		String dim = mc.level.dimension().identifier().toString();
		// 1.21.11 dropped Camera.getYRot(); the yaw is derived from the camera's
		// forward vector in capture() instead. Before the first world frame lands,
		// fall back to the player's own yaw so the bar is never blank.
		double viewYaw = valid ? camYawDeg : mc.player.getYRot();
	...
	private static void barGem(GuiGraphics g, double cx, double cy, float s, int color, int alpha) {
		...
		pose.translate((float) (cx - 0.5 * s), (float) (cy - 0.5 * s));
		pose.scale(s, s);
	...
	private static int pillText(GuiGraphics g, Font font, int cx, int y, String text,
		...
		var pose = g.pose();
		pose.pushMatrix();
		pose.translate((float) cx, (float) y);
		pose.scale(s, s);
