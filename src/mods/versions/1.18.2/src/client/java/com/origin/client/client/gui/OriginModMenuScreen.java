...
public class OriginModMenuScreen extends Screen {
	...
	public OriginModMenuScreen() {
		super(new TextComponent("Origin Mods"));
	}
	...
	private void renderGrid(Gfx g, int mouseX, int mouseY, long now, float alpha) {
		int hy = py() + 10;
	...
	private void renderTabs(Gfx g, int mx, int my, int hy, float alpha) {
	...
	private void renderSettingsTab(Gfx g, int mx, int my, long now, float alpha) {
	...
	// ---- backing-store dispatch: the "jei" page reads/writes JEI's own config
	// (JeiSettings); every other page uses ModsConfig (Mods.*). One switch per access. ----
	private static boolean isJei(String id) { return "jei".equals(id); }
	private java.util.List<ModOption> optionsFor(Mods.Mod mod) { return isJei(mod.id()) ? JeiSettings.options() : mod.options(); }
	private boolean vBool(String id, String key) { return isJei(id) ? JeiSettings.getBool(key) : Mods.bool(id, key); }
	private void vSetBool(String id, String key, boolean v) { if (isJei(id)) { JeiSettings.setBool(key, v); } else { Mods.set(id, key, v); } }
	private double vNum(String id, String key) { return isJei(id) ? JeiSettings.getNum(key) : Mods.num(id, key); }
	private void vSetNum(String id, String key, double v) { if (isJei(id)) { JeiSettings.setNum(key, v); } else { Mods.set(id, key, v); } }
	private String vMode(String id, String key) { return isJei(id) ? JeiSettings.getMode(key) : Mods.mode(id, key); }
	private void vSetMode(String id, String key, String v) { if (isJei(id)) { JeiSettings.setMode(key, v); } else { Mods.set(id, key, v); } }
	private String vMulti(String id, String key) { return isJei(id) ? JeiSettings.getMulti(key) : Mods.mode(id, key); }
	private void vSetMulti(String id, String key, String csv) { if (isJei(id)) { JeiSettings.setMulti(key, csv); } else { Mods.set(id, key, csv); } }
	private int multiBtnW(int x0, int x1) { return Math.min(180, Math.max(80, (x1 - x0) / 2)); }
	private static java.util.List<String> splitCsv(String csv) {
		java.util.List<String> out = new java.util.ArrayList<>();
		if (csv != null) { for (String q : csv.split(",")) { String t = q.trim(); if (!t.isEmpty()) { out.add(t); } } }
		return out;
	}
	private static String joinCsv(java.util.List<String> tokens) { return String.join(", ", tokens); }
	...
	private void renderRow(Gfx g, String modId, ModOption o, int x0, int x1, int y, int mx, int my, float alpha) {
		...
		switch (o.kind) {
			...
			case MULTISELECT -> {
				...
				String summary = sel.isEmpty() ? "None" : String.join(", ", sel.stream().map(JeiSettings::prettify).toList());
				int bw = multiBtnW(x0, x1);
				int bx = x1 - 10 - bw;
				boolean bHover = in(mx, my, bx, y + 4, bx + bw, y + 22);
				OriginUi.panel(g, bx, y + 4, bw, 18, 7, withAlpha(bHover ? 0x2EFFFFFF : 0x1EFFFFFF, alpha), withAlpha(OriginTheme.STROKE, alpha));
				String shown = font.width(summary) > bw - 16 ? font.plainSubstrByWidth(summary, bw - 22) + "…" : summary;
		...
		switch (o.kind) {
			...
			case MULTISELECT -> {
				...
				if (mx >= bx && mx <= x1 - 10) {
					...
					java.util.List<String> sel = splitCsv(vMulti(modId, o.key));
					String mid = modId; String mkey = o.key;
					OriginMultiSelect.open(o.label, allChoices, sel, chosen -> vSetMulti(mid, mkey, joinCsv(chosen)));
					return true;
				}
			}
		}
