...
// PER-VERSION DELTA (1.21.6 - 1.21.10): this era's DynamicTransforms block ends
// with a fifth member, LineWidth, which 1.21.11 dropped. A std140 block must be
// declared IDENTICALLY in both stages — with the member missing the program fails
// to link ("unknown stage block member has no corresponding member in fragment
// block") and the grade silently never draws. Verified against each era's own
