...
public final class OriginMultiSelect {
	...
	public static void render(GuiGraphicsExtractor g, int mx, int my) {
		...
		g.text(font, title == null ? "Select" : title, px + 12, py + 10, OriginTheme.TEXT, false);
		...
		for (int i = 0; i < selected.size(); i++) {
			...
			g.text(font, String.valueOf(i + 1), x0 + 6, y + 5, OriginTheme.MUTED, false);
			g.text(font, raw, x0 + 20, y + 5, OriginTheme.TEXT, false);
		...
		// divider
		g.text(font, selected.isEmpty() ? "PICK ONE OR MORE" : "ADD MORE",
		...
		for (String raw : avail) {
			...
			g.text(font, raw, x0 + 8, y + 5,
					rowHover ? OriginTheme.TEXT : OriginTheme.TEXT_DIM, false);
			g.text(font, "+", x1 - 14, y + 5, rowHover ? OriginTheme.TEXT : OriginTheme.MUTED, false);
			y += ROW;
		}
	}

	private static void drawGlyph(GuiGraphicsExtractor g, Font font, String s, int x, int y, int mx, int my, boolean enabled) {
		...
		g.text(font, s, x, y + 5, color, false);
	}
