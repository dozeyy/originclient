...
public class OriginModMenuScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
		...
		var pose = g.pose();
		pose.pushMatrix();
		pose.translate(0, (float) ((1.0 - p) * (height - py())));
		...
		pose.translate((float) (px() + pw() / 2.0), (float) (py() + ph() / 2.0));
		pose.scale(s, s);
		pose.translate((float) (-(px() + pw() / 2.0)), (float) (-(py() + ph() / 2.0)));
		...
		g.text(font, ver, px() + pw() - 10 - font.width(ver), py() + ph() - 13, withAlpha(OriginTheme.MUTED, 0.8f), false);
	...
	private void drawTooltip(GuiGraphicsExtractor g, int mx, int my, String text) {
		...
		int ty = by + 5;
		for (String l : lines) {
			g.text(font, l, bx + 6, ty, OriginTheme.TEXT, false);
			ty += lh;
		}
	}
	...
	private void renderGrid(GuiGraphicsExtractor g, int mouseX, int mouseY, long now, float alpha) {
		int hy = py() + 10;
		...
		g.text(font, "HUD Editor", hbX + 8, hy + 6, withAlpha(OriginTheme.TEXT, alpha), clear);
		...
		if (search.isEmpty() && !searchFocused) {
			g.text(font, "Search mods", sx + 24, sy + 7,
					withAlpha(clear ? OriginTheme.TEXT_DIM : OriginTheme.MUTED, alpha), clear);
		} else {
			g.text(font, search, sx + 24, sy + 7, withAlpha(OriginTheme.TEXT, alpha), clear);
		}
		if (searchFocused) {
	...
	private void renderTabs(GuiGraphicsExtractor g, int mx, int my, int hy, float alpha) {
	...
	private void drawTab(GuiGraphicsExtractor g, int tx, int ty, int w, int h, String label, boolean active, boolean hover, float alpha) {
		...
		g.text(font, label, tx + (w - font.width(label)) / 2, ty + (h - 8) / 2,
	...
	private void renderCard(GuiGraphicsExtractor g, Mods.Mod mod, int[] r, int mx, int my, float alpha) {
		...
		g.text(font, name, cx + (cellW - font.width(name)) / 2, cy + 47, withAlpha(OriginTheme.TEXT, alpha), clear);
		...
		g.text(font, "OPTIONS", bx + (bw - font.width("OPTIONS")) / 2, oby + 4,
		...
		g.text(font, label, bx + (bw - font.width(label)) / 2, tby + 4,
				withAlpha(on ? GREEN_TEXT : RED_TEXT, alpha), false);
	}

	private void renderSettingsTab(GuiGraphicsExtractor g, int mx, int my, long now, float alpha) {
	...
	private void renderSettings(GuiGraphicsExtractor g, int mouseX, int mouseY, long now, float alpha, Mods.Mod mod) {
		...
		g.text(font, "<", x0 + 9, hy + 6, withAlpha(OriginTheme.TEXT, alpha), false);

		OriginUi.icon(g, mod.id(), x0 + 32, hy - 3, 26, withAlpha(OriginTheme.TEXT, alpha));
		g.text(font, mod.name(), x0 + 64, hy + 2, withAlpha(OriginTheme.TEXT, alpha), false);
		if (!mod.description().isEmpty()) {
			g.text(font, mod.description(), x0 + 64, hy + 13, withAlpha(OriginTheme.MUTED, alpha), false);
		}
		...
		if (!settingsSearch.isEmpty()) {
			g.text(font, settingsSearch, x0 + 22, sby + 6, withAlpha(OriginTheme.TEXT, alpha), false);
		}
		...
		if (mod.options().isEmpty()) {
			g.text(font, "No additional settings — the switch is everything.",
	...
	private void drawRows(GuiGraphicsExtractor g, String id, int x0, int x1, int top, int bottom, int mouseX, int mouseY, float alpha) {
		for (SRow r : srows) {
			...
			if (r.o().kind == ModOption.Kind.HEADER) {
				g.text(font, r.o().label.toUpperCase(java.util.Locale.ROOT), x0 + 2, r.y() + 5,
	...
	private void renderRow(GuiGraphicsExtractor g, String modId, ModOption o, int x0, int x1, int y, int mx, int my, float alpha) {
		...
		switch (o.kind) {
			...
			case SLIDER -> {
				...
				g.text(font, val, tx - font.width(val) - 10, y + 9, withAlpha(OriginTheme.TEXT, alpha), false);
			}
			case COLOR -> {
				...
				g.text(font, hex, cx - 8 - font.width(hex), y + 9, withAlpha(OriginTheme.TEXT_DIM, alpha), false);
			}
			case HEADER -> {
			}
			case KEYBIND -> {
				...
				g.text(font, name, x1 - 10 - bw + 8, y + 9, withAlpha(OriginTheme.TEXT, alpha), false);
			}
			case DROPDOWN -> {
				...
				g.text(font, "<", bx + 6, y + 9, withAlpha(OriginTheme.TEXT_DIM, alpha), false);
				g.text(font, v, bx + (bw - font.width(v)) / 2, y + 9, withAlpha(OriginTheme.TEXT, alpha), false);
				g.text(font, ">", bx + bw - 6 - font.width(">"), y + 9, withAlpha(OriginTheme.TEXT_DIM, alpha), false);
			}
		}
	}
