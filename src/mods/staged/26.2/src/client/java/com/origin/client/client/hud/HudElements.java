...
public final class HudElements {
	...
	// Set while the HUD editor previews the scoreboard itself, so the normal HUD
	// path does not draw a second copy.
	public static volatile boolean suppressScoreboard = false;

	public interface Renderer {
		void render(GuiGraphicsExtractor g, Minecraft mc, int w, int h);
	}
	...
	private static boolean chatOpen(Minecraft mc) {
		return com.origin.client.client.OriginScreenState.current instanceof ChatScreen;
	}

	/** Standard text row backing, gated by the mod's Show Background toggle. */
	private static void bg(GuiGraphicsExtractor g, String modId, int w, int h) {
		if (Mods.bool(modId, "showBackground")) {
			OriginUi.panel(g, -3, -3, w + 6, h + 6, 5, PANEL, 0);
		}
	}

	private static void bgColored(GuiGraphicsExtractor g, String modId, String colorKey, int w, int h) {
		if (Mods.bool(modId, "showBackground")) {
	...
	private static List<String> coordsLines(Minecraft mc) {
		...
		if (Mods.bool("coords", "renderers")) {
			int c;
			try {
				c = 0; // 26.2 STUB: countRenderedSections() removed (HUD deferred)
			} catch (Throwable t) {
				c = -1;
			}
	...
	// The sign of the dominant horizontal axis of the look direction — whether walking
	// forward makes that coordinate go up (+) or down (-).
	...
	// A representative colour per biome family so the name reads in a matching tone.
	...
	private static List<ItemStack> armorItems(Minecraft mc) {
		...
		if (mc.player != null) {
			// Helmet-first, then held item. 26.2 removed the Inventory.armor list;
			// getItemBySlot(EquipmentSlot) is the per-slot accessor.
			for (net.minecraft.world.entity.EquipmentSlot slot : new net.minecraft.world.entity.EquipmentSlot[]{
					net.minecraft.world.entity.EquipmentSlot.HEAD,
					net.minecraft.world.entity.EquipmentSlot.CHEST,
					net.minecraft.world.entity.EquipmentSlot.LEGS,
					net.minecraft.world.entity.EquipmentSlot.FEET}) {
				ItemStack piece = mc.player.getItemBySlot(slot);
				if (!piece.isEmpty()) {
					items.add(piece);
				}
			}
	...
	static {
		...
		add("fps", "fps", "FPS", new HudPos(0, 6, 6, 1.0), mc -> {
			...
			g.text(mc.font, fpsText(mc), 1, 1, OriginColorPicker.liveColor("fps", "color"), Mods.bool("fps", "textShadow"));
			p.popMatrix();
		});
		...
		add("cps", "cps", "CPS", new HudPos(0, 6, 22, 1.0), mc -> {
			...
			g.text(mc.font, cpsText(), 1, 1, OriginColorPicker.liveColor("cps", "color"), Mods.bool("cps", "textShadow"));
			p.popMatrix();
		});
		...
		add("coords", "coords", "Coords", new HudPos(0, 6, 38, 1.0), mc -> {
			...
			// them even when some axes are switched off (hardcoding rows 2/3 misaligned
			// the moment X/Y/Z were toggled).
			int yRowY = -1, zRowY = -1;
			for (String line : coordsLines(mc)) {
				...
				if (line.startsWith("Biome: ")) {
					// "Biome: " label in the normal tone, the biome NAME in its family colour.
					String label = "Biome: ";
					String bname = line.substring(label.length());
					g.text(mc.font, label, 1, y, TEXT, shadow);
					g.text(mc.font, bname, 1 + mc.font.width(label), y, biomeColor(bname), shadow);
					y += 10;
					continue;
				}
				g.text(mc.font, line, 1, y, TEXT, shadow);
				y += 10;
			}
			// Direction column (vertical mode): the 8-way letter sits beside the Y row
			// and the travel sign ("+" heading South/East, "-" heading North/West)
			// beside the Z row, both right-aligned — matching the reference layout.
			// Horizontal mode already folded them into the text.
			if (Mods.bool("coords", "direction") && !Mods.mode("coords", "listMode").equals("Horizontal")) {
				...
				g.text(mc.font, letter, right - mc.font.width(letter), ly, TEXT, shadow);
				g.text(mc.font, sign, right - mc.font.width(sign), sy2, TEXT, shadow);
			}
			p.popMatrix();
		});
		...
		add("serveraddress", "serveraddress", "Server IP", new HudPos(2, -6, 6, 1.0), mc -> {
			...
			g.text(mc.font, serverText(mc), x, 2, color, shadow);
			p.popMatrix();
		});
	...
	private static List<PotionRow> potionRows(Minecraft mc) {
		...
		if (rows.isEmpty() && editorPreview) {
			rows.add(new PotionRow(MobEffects.SPEED, 83 * 20, false));
			rows.add(new PotionRow(MobEffects.STRENGTH, 45 * 20, false));
		}
		return rows;
	}

	private static void drawEffectRow(GuiGraphicsExtractor g, Minecraft mc, int y, Holder<MobEffect> effect, int duration, boolean infinite) {
		...
		try {
			// The real effect icon (textures/mob_effect/<id>.png, 18x18), drawn at
			// 16x16 beside the row. 26.2 dropped MobEffectTextureManager, so we blit
			// the texture directly by its registry id.
			var key = net.minecraft.core.registries.BuiltInRegistries.MOB_EFFECT.getKey(effect.value());
			if (key != null) {
				var tex = net.minecraft.resources.Identifier.fromNamespaceAndPath(
						key.getNamespace(), "textures/mob_effect/" + key.getPath() + ".png");
				g.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, tex,
						2, y + 2, 0f, 0f, 16, 16, 18, 18, 18, 18, 0xFFFFFFFF);
			} else {
		...
		int x = 22;
		if (!showName) {
			g.text(mc.font, time, x, y + 5, timeColor, shadow);
			return;
		}
		String nm = name;   // full name, never trimmed (the box grows to fit)
		if (Mods.bool("potionhud", "reversedText")) {
			g.text(mc.font, time, x, y + 5, timeColor, shadow);
			g.text(mc.font, nm, x + mc.font.width(time + " "), y + 5, nameColor, shadow);
		} else {
			g.text(mc.font, nm, x, y + 5, nameColor, shadow);
			g.text(mc.font, time, x + mc.font.width(nm + " "), y + 5, timeColor, shadow);
		}
	}

	// ---- armor ----

	private static void renderArmor(GuiGraphicsExtractor g, Minecraft mc, int w, int h) {
		if (mc.player == null) {
			return;
		}
	...
	private static void renderKeystrokes(GuiGraphicsExtractor g, Minecraft mc, int w, int h) {
		float s = ks();
		var p = g.pose();
		p.pushMatrix();
		p.scale(s, s);
		p.translate(0, -ksBounds()[0]); // hug the topmost shown row
		var o = mc.options;
	...
	private static void key(GuiGraphicsExtractor g, Minecraft mc, int x, int y, int w, int h, String label, boolean down) {
		...
		if (!label.isEmpty()) {
			...
			g.text(mc.font, label, x + (w - tw) / 2, y + (h - 8) / 2, tc, Mods.bool("keystrokes", "textShadow"));
		}
	}

	/** Per-module rounded backing at the element's own opacity (0 = none). */
	public static void drawBacking(net.minecraft.client.gui.GuiGraphicsExtractor g, int x, int y, int w, int h, double bg) {
		if (bg <= 0.01) {
			return;
		}
		int a = (int) (bg * 255);
	...
	public static void renderInInventory(GuiGraphicsExtractor g) {
		...
		for (Element e : ALL) {
			...
			p.scale((float) pos.scale, (float) pos.scale);
			try {
	...
	public static void renderAll(GuiGraphicsExtractor g) {
		Minecraft mc = Minecraft.getInstance();
		if (mc.player == null || false /* 26.2 STUB: Options.hideGui relocated (HUD deferred) */ || com.origin.client.client.OriginScreenState.current instanceof HudEditorScreen) {
			return;
		}
